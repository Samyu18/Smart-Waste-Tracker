...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///waste_tracker.db'
...
def get_category_map(user_id=None):
    if user_id:
...
def get_expiry_queue(user_id=None):
    if user_id:
        items = Item.query.filter_by(user_id=user_id).all()
    else:
        items = Item.query.all()
...
def forecast_surplus(window_days=7, user_id=None):
    if user_id:
        items = Item.query.filter_by(user_id=user_id).all()
    else:
        items = Item.query.all()
...
def dashboard():
    ...
    surplus = forecast_surplus(user_id=session['user_id'])
    expiry_pq = get_expiry_queue(user_id=session['user_id'])
...
def analytics():
    items = Item.query.filter_by(user_id=session['user_id']).all()
    category_map = get_category_map(user_id=session['user_id'])
